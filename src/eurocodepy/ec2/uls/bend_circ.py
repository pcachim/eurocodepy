...
epsc2 = 2 if fck <= 50 else 2.0 + 0.085 * np.pow(fck - 50.0, 0.53)
epscu2 = 3.5 if fck <= 50 else 2.5 + 35 * np.pow((90.0 - fck)/100.0, 4)
n = 2 if fck <= 50 else 1.4 + 23.4 * np.pow((90.0 - fck)/100.0, 4)
x2 = 1.0 - epsc2/epscu2
...
def steelforces(x0: float, ns: int = 6):
    epsyd = fyd / Es * 1000
...
if __name__ == "__main__":
    ...
    props2 = {'boxstyle': 'round', 'pad': 0, 'ec': "white", 'facecolor': 'white', 'alpha': 0.8}
    props3 = {'boxstyle': 'round', 'pad': 0, 'ec': "white", 'facecolor': 'white', 'alpha': 0.8}
