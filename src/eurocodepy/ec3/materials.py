...
class Bolt:
    ...
    @staticmethod
    def M20(grade: str = "8.8") -> "Bolt":  # noqa: N802
        """Create a Bolt instance with M20 diameter.
    ...
    def M22(grade: str = "8.8") -> "Bolt":
        """Create a Bolt instance with M22 diameter.
    ...
    def M24(grade: str = "8.8") -> "Bolt":
        """Create a Bolt instance with M24 diameter.
    ...
    def M27(grade: str = "8.8") -> "Bolt":
        """Create a Bolt instance with M27 diameter.
    ...
    def M30(grade: str = "8.8") -> "Bolt":
        """Create a Bolt instance with M30 diameter.
...
class BoltedConnection:
    ...
    def e2(self, value: float) -> None:
        ...
        ------
        ValueError
            If provided value is less than the minimum or exceeds the maximum allowed.
    ...
    def p2(self, value: float) -> None:
        ...
        if value < MIN_P2 * self.d0:
            msg = (
                f"Spacing perpendicular to force must be at least {MIN_P2} "
                f"times the hole diameter ({self.d0})."
            )
            raise ValueError(msg)
        t = min(self.steel_plate.thickness, self.steel_plate_b.thickness)
    ...
    def n1(self, value: int) -> None:
        """Set the number of bolts in parallel.
    ...
    def n2(self, value: int) -> None:
        """Set the number of bolts in series.
    ...
    def Fb_Rd(self) -> float | tuple:  # noqa: N802
    ...
    def Ft_Rd(self, countersunk: bool = False) -> float:  # noqa: N802
        ...
        return np.round(k2 * self.bolt.fub * self.bolt.Athread /
                        self.bolt.gamma_M2 / 10.0, 2)
    ...
    def check(self, Fv_Ed: np.ndarray, Ft_Ed: np.ndarray,  # noqa: N803
        ...
        combined_ratio = np.round(self.Fv_Ed / self.Fv_Rd() +
                                self.Ft_Ed / 1.4 / self.Ft_Rd(), 2)
...
class PinnedConnection(BoltedConnection):
    ...
    def Fv_Rd(self, threaded: bool = False) -> float:  # noqa: N802
        """Return shear strength.
    ...
    def Ft_Rd(self, countersunk: bool = False) -> float:  # noqa: N802, PLR6301
...
class WeldConnection:
    """Represents a welded connection with multiple welds according to Eurocode 3.

    Attributes
    ----------
    welds : list
        List of Weld objects in the connection.
    inertia_xx : float
        Second moment of inertia about x-axis (mm^4).
    inertia_yy : float
        Second moment of inertia about y-axis (mm^4).
    inertia_xy : float
        Product of inertia (mm^4).
    area_xx : float
        Effective area in x-direction (mm^2).
    area_yy : float
        Effective area in y-direction (mm^2).
    area : float
        Total throat area of all welds (mm^2).
    max_x : float
        Maximum x-coordinate of all welds (mm).
    max_y : float
        Maximum y-coordinate of all welds (mm).
    min_x : float
        Minimum x-coordinate of all welds (mm).
    min_y : float
        Minimum y-coordinate of all welds (mm).

    Methods
    -------
    add(weld: Weld)
        Add a weld to the connection.
    total_length()
        Calculate the total length of all welds.
    ...
    def __init__(self, welds: list | None = None):  # noqa: ANN204, D107
        self.welds = welds if welds is not None else []

    def add(self, weld: Weld) -> None:
        """Add a weld to the connection.

        Parameters
        ----------
        weld : Weld
            The weld object to add to the connection.
    ...
    def total_length(self) -> float:
        """Calculate the total length of all welds in the connection.

        Returns:
            float: The sum of lengths of all welds in the connection (mm).

        """
        return sum(weld.length for weld in self.welds)

    def __len__(self) -> float:
        """Return the number of welds in the connection.

        Returns:
            int: The number of welds in the connection.

        """
        return len(self.welds)

    def __iter__(self):  # noqa: ANN204
        """Return an iterator over the welds in the connection.

        Returns:
            iterator: An iterator over the welds list.

        """
        return iter(self.welds)

    def __repr__(self) -> str:
        """Return a string representation of the WeldConnection instance.

        Returns:
            str: A string representation of the WeldConnection instance.
